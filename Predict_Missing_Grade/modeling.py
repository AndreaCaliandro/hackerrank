...
class RunningModel():
    ...
    def running_model(self):
        X_train_unscaled, X_test_unscaled, self.y_train, self.y_test = \
            train_test_split(self.Features, self.Target, test_size=0.1, random_state=1)
        if self.scaling:
    ...
    def score(self, cm):
        correct = 0  # cm[0][0:1].sum() + cm[1][0:2] + cm[2][1:3]
        for i in range(8):
            correct += cm[i][max(0,i-1):min(i+2,8)].sum()
        total = cm.sum()
        wrong = total - correct
